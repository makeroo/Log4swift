...
@objc public final class LoggerFactory: NSObject {
  ...
  @objc public func getLogger(_ identifierToFind: String) -> Logger {
    ...
    var currentLogger = self.rootLogger

    func resolveLogger (_ name: String) -> Logger {
        if let l = self.loggers[name] {
            return l
        }

        let l = Logger(parentLogger: currentLogger, identifier: name)
    ...
    var lastIndex = identifierToFind.startIndex

    for c in identifierToFind {
        if c == "." {
            let stepName = String(identifierToFind[..<lastIndex])

            currentLogger = resolveLogger(stepName)
        }

        lastIndex = identifierToFind.index(after: lastIndex)
    }

    return resolveLogger(identifierToFind)
  }
